...
from apis.telnet_cmd import telnet_cmd
from config import logger


class Devices_Command(BaseModel):
    command: str = None
    deviceIds: str
...
class Cron:
    ...
    def cron(self, command_id=None):
        if command_id == 'all':
            ...
            sql = "select %s from %s where command_id=%s" % (
                'device_id, command, command_id', 'device_config_command_tb', command_id)
        ...
        for command in commands:
            ...
            sql = 'select ip_adress, user_name, device_password, factory_id,login_function from device_tb where device_id = %s' % command[
                0]
            # logger.info(sql)
            ...
            try:
                ...
                try:
                    ...
                    sql = 'insert into device_config_tb (device_id,command_id,config_content,config_version) values ("{0}", "{1}", "{2}", "{3}")'.format(
                        command[0], command[2],
                        tmp_result.get('result').replace('"', '\\"'), str(time.ctime()))
                # logger.info(sql)
                self._insert(sql)
    ...
    def devices_test_cmd(self, _dict):
        result = {}
        for device_id in _dict.deviceIds.split(','):
...
async def cron_device_conf(devices_commands: Devices_Command):
    return Cron().devices_test_cmd(devices_commands)


async def cron_command_id(command_id: str):
    return Cron().cron(str(command_id))


async def crons():
    return Cron().cron('all')
